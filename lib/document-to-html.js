...
function DocumentConversion(options, comments) {
    ...
    var defaultParagraphStyle = function(attributes) {
        return htmlPaths.topLevelElement("p", attributes);
    };
    ...
    function elementToHtml(element, messages, options) {
        ...
        if (element.type === 'commentRangeStart') {
            withinComments.push(element);
            return [];
        } else if (element.type === 'commentRangeEnd') {
            withinComments = withinComments.filter(function(elm) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            ...
            return defaultParagraphStyle({style: buildStyles(element)});
        }
    }

    function buildStyles(element) {
        var styles = {};
        if (element.fontSize) {
            styles['font-size'] = `${element.fontSize / 2 >> 0}pt`;
        }
        if (element.alignment) {
            styles['text-align'] = element.alignment;
        }
        if (element.indent.start || element.indent.firstLine) {
            styles['padding-left'] =
                `${(element.indent.start + element.indent.firstLine) / 20 >> 0}px`;
        }
        return Object.entries(styles).map(function([key, val]) {
            return `${key}: ${val}`;
        }).join(';');
    }

    function convertInsert(element, messages, options) {
    ...
    function convertDelete(element, messages, options) {
        return Html.freshElement(
            "del", {
                date: element.date,
                author: element.author
    ...
    function convertCommentRange(element, children) {
        return [Html.freshElement('span', {class: 'comment', 'data-id': element.id}, children)];
    }

    function convertText(element, messages, options) {
        return withinComments.reverse().reduce(function (prev, curr) {
            return convertCommentRange(curr, prev);
    ...
    function convertTable(element, messages, options) {
        ...
        if(htmlPath._elements.length > 0) {
            htmlPath._elements[0].attributes.border = new String(element.border);
            if(element.width) {
                htmlPath._elements[0].attributes.style = new String('width: ' + element.width + 'px;');
            }
        }

        return htmlPath.wrap(function(elem) {
    ...
    var elementConverters = {
        ...
        "commentRangeStart": function(element, messages, options) {
            return Html.freshElement("span", {class: 'comment-start', 'data-id': element.id}, [Html.forceWrite]);
        },
        "commentRangeEnd": function(element, messages, options) {
            return Html.freshElement("span", {class: 'comment-end', 'data-id': element.id}, [Html.forceWrite]);
        },
