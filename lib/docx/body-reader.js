...
function BodyReader(options) {
    ...
    function readTableProperties(element) {
        var styles = readTableStyle(element).map(function(style) {
            return {
                styleId: style.styleId,
                styleName: style.name
            };
        ...
        if(tableBordersElement) {
            for(var i = 0, _l = tableBordersElement.children.length; i < _l; i++) {
                ...
                if(borderChildElement.attributes['w:sz'] && parseInt(borderChildElement.attributes['w:sz']) > 0) {
                    styles.value.border = Math.round(parseInt(borderChildElement.attributes['w:sz']) / 8); // 1/2 point
                    break;
                }
            }
        }
